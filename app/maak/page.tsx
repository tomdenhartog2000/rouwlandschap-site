...
export default function MaakEenRouwdier() {
  ...
  const createAiImage = async (direction = aiPrompt) => {
    setIsGenerating(true);
    setGenerationError("");
    try {
      const data = new FormData();
      data.set("direction", direction);
      data.set("context", [words, reference, careReflection].filter(Boolean).join("\n"));
      const source = photos[0]?.dataUrl || drawingDataUrl;
  ...
  const finishContribution = async () => {
    ...
    try {
      ...
      if (aiImageDataUrl) data.set("aiImage", await dataUrlToFile(aiImageDataUrl, "ai-toevoeging.png"));
