...
export default function MaakEenRouwdier() {
  ...
  return <main className="make-page"><header className="make-header"><a href="/verken" className="back-link">← terug naar het landschap</a></header><section className="make-card" aria-live="polite">
    {step === 1 && <div><p className="eyebrow">1 van 5 · iets meenemen</p><h1>Waar maak je jouw rouwdier van?</h1><p className="lead">Misschien is er al iets: een gewoonte, een voorwerp, een plek, een gezegde, een moment dat blijft terugkomen. Hier geef je dat een vorm. Het hoeft niet nieuw te zijn, niet af, en niet mooi.</p><div className="input-groups">{inputGroups.map((group) => <section className="input-group" key={group.id} aria-labelledby={`input-group-${group.id}`}><h2 id={`input-group-${group.id}`}>{group.title}</h2><div className="input-options">{group.inputIds.map((inputId) => { const input = inputs.find((item) => item.id === inputId)!; const isSelected = modes.includes(input.id); return <button type="button" key={input.id} aria-pressed={isSelected} className={`input-option ${isSelected ? "is-selected" : ""}`} onClick={() => toggleMode(input.id)}><span className={`input-symbol ${input.symbol === "camera" ? "input-symbol-camera" : input.symbol === "microphone" ? "input-symbol-microphone" : ""}`} aria-hidden="true">{input.symbol !== "camera" && input.symbol !== "microphone" ? input.symbol : <span />}</span><span><strong>{input.title}</strong><small>{input.text}</small></span><span className="input-state">{isSelected ? "toegevoegd" : "voeg toe"}</span></button>; })}</div></section>)}</div>{modes.length ? <div className="input-surfaces">{modes.map(renderInput)}</div> : <p className="empty-input-message">Je kunt iets kiezen, of meteen verdergaan.</p>}<div className="step-actions"><a className="quiet-button" href="/">terug</a><button type="button" className="primary-button" onClick={() => setStep(2)}>verder</button></div></div>}
