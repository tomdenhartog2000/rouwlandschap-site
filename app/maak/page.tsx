...
export default function MaakEenRouwdier() {
  ...
  const addPhotos = (files?: FileList | null) => { const selectedFiles = Array.from(files ?? []); if (selectedFiles.length) setPhotos((current) => [...current, ...selectedFiles.map((file) => ({ name: file.name, url: URL.createObjectURL(file) }))]); };
