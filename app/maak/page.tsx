...
export default function MaakEenRouwdier() {
  ...
  const hasOriginalVisual = Boolean(photos.length || (modes.includes("draw") && drawingDataUrl));
  ...
  const finishContribution = async () => {
    ...
    try {
      ...
      if (!hasAiEndProduct || sourceVisualChoice === "with-source") {
  ...
  const downloadCard = async () => {
    ...
    const sourceVisualSource = sourceVisualChoice === "with-source" ? photos[0]?.dataUrl || drawingDataUrl : "";
