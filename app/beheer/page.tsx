...
type ManagedLandscape = { id: string; name: string; active: number };
...
export default function Beheer() {
  ...
  const addLandscape = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); const response = await fetch("/api/admin/landscapes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: landscapeName }) }); if (!response.ok) { setMessage("Dit landschap kon niet worden toegevoegd."); return; } setLandscapeName(""); await load(); };
  ...
  return <main className="admin-page"><section className="admin-shell"><header><div><p className="eyebrow">beheer</p><h1>Rouwdieren in het online landschap</h1><p>{items.length === 1 ? "1 rouwdier" : `${items.length} rouwdieren`}</p></div><a href="/verken" className="quiet-button">bekijk landschap</a></header>{message && <p className="admin-message">{message}</p>}<section className="admin-landscapes"><h2>Actief landschap</h2><p>Nieuwe rouwdieren worden aan dit landschap toegevoegd.</p>{landscapes.map((landscape) => <button key={landscape.id} type="button" className={landscape.active ? "is-selected" : "secondary-button"} onClick={() => void activateLandscape(landscape.id)}>{landscape.active ? `${landscape.name} · actief` : `maak ${landscape.name} actief`}</button>)}<form onSubmit={addLandscape}><label>nieuw landschap<input value={landscapeName} onChange={(event) => setLandscapeName(event.target.value)} placeholder="bijvoorbeeld: Afstudeerlandschap" /></label><button className="secondary-button">voeg toe</button></form></section><div className="admin-list">{items.length === 0 && <p className="admin-empty">Er zijn nog geen gedeelde rouwdieren.</p>}{items.map((item) => <article key={item.id} className={`admin-item ${item.status === "hidden" ? "is-hidden" : ""}`}><div className="admin-item-head"><div><p>{item.kind} · {item.status === "visible" ? "zichtbaar" : "verborgen"}</p><h2>{item.title}</h2></div><div className="admin-actions"><button type="button" className="secondary-button" disabled={busyId === item.id} onClick={() => void update(item.id, "toggle")}>{item.status === "visible" ? "verberg" : "maak zichtbaar"}</button><button type="button" className="quiet-button admin-delete" disabled={busyId === item.id} onClick={() => void update(item.id, "delete")}>verwijder</button></div></div>{item.attachments.filter((attachment) => attachment.role !== "audio").map((attachment) => <img key={attachment.name} src={attachment.url} alt="Bijdrage van de bezoeker" />)}{item.attachments.filter((attachment) => attachment.role === "audio").map((attachment) => <audio key={attachment.name} controls src={attachment.url}>Je browser kan deze opname niet afspelen.</audio>)}{item.text && <p className="admin-text">{item.text}</p>}{item.description && item.description !== item.text && <p className="admin-text">{item.description}</p>}{item.reference && <p className="admin-reference">{item.reference}</p>}{item.referenceLink && <a href={item.referenceLink} target="_blank" rel="noreferrer">open verwijzing</a>}</article>)}</div></section></main>;
}
