...
export async function POST(request: Request) {
  ...
  try {
    ...
    const prompt = [
      ...
      aiPath === "together" && isRevision && sourceFile ? "Treat the supplied image as the current co-created result. Keep its recognisable subject, structure, lines, proportions, and all elements the visitor did not ask to change. Apply only the requested adjustment and return one integrated image." : aiPath === "together" && sourceFile && sourceKind === "drawing" ? "Edit the supplied drawing into one integrated co-created image. The visitor's drawn line is the essential structural source: follow the same path, turns, proportions, placement, and overall silhouette closely. Keep it clearly recognisable in the result. You may gently smooth uneven strokes and add subtle requested visual elements around or within it, but do not replace it with a different line, object, symbol, or composition." : aiPath === "together" && sourceFile ? "Edit the supplied first photo into one integrated co-created image. Keep the original subject and composition clearly recognisable and change or add only what the visitor requests. Do not replace it with an unrelated image." : sourceFile && sourceKind === "drawing" ? "Use the supplied drawing as the structural source for a new visual version. Follow the same path, turns, proportions, and overall silhouette closely. You may gently smooth uneven strokes and make the linework more fluent, but do not invent a different line, object, symbol, or composition." : sourceFile ? "Use the first supplied photo as source material. Create a fully developed new visual interpretation from it. It may be expanded, transformed, and re-composed while remaining clearly connected to that photo." : "Make a visual starting point from the visitor's words without illustrating them literally.",
