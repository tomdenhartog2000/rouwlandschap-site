...
export async function POST(request: Request) {
  ...
  try {
    ...
    const requestedSourceKind = incoming.get("sourceKind");
    const sourceKind = requestedSourceKind === "drawing" || requestedSourceKind === "sounddrawing" || requestedSourceKind === "photo" ? requestedSourceKind : "none";
    ...
    const isDrawingSource = sourceKind === "drawing" || sourceKind === "sounddrawing";
    ...
    const prompt = [
      isConstrainedEdit ? "Perform one precise, constrained edit to the supplied image for a participatory art project about living with loss." : aiPath === "translate" ? "Create one fully developed and distinctly new visual interpretation for a participatory art project about living with loss." : "Create one quiet visual addition for a participatory art project about living with loss.",
      isRevision ? "Treat the supplied image as the current AI result. Keep its recognisable subject, structure, lines, proportions, placement, composition, and every element the visitor did not ask to change. Apply exactly and only the requested adjustment and return one integrated image." : isConstrainedEdit && isDrawingSource ? "Edit the supplied drawing into one integrated co-created image. The visitor's drawn line is the essential structural source: follow the same path, turns, proportions, placement, and overall silhouette closely. Preserve the composition. You may minimally smooth uneven strokes without moving or replacing the line. Apply exactly and only the change the visitor explicitly requests." : isConstrainedEdit ? "Edit the supplied first photo into one integrated co-created image. Keep the original subject, proportions, placement, and composition clearly recognisable. Apply exactly and only the change the visitor explicitly requests." : sourceFile && sourceKind === "sounddrawing" ? "Treat the supplied sound-drawing as a visual trace of movement, rhythm, and sound, not as a contour that must be copied. Freely transform its gesture and energy into a new, complete visual world that follows the visitor's direction. The result should clearly go beyond a cleaned-up or recoloured version of the line." : sourceFile && sourceKind === "drawing" ? "Treat the supplied drawing as inspiration and source material, not as a contour that must be preserved. Freely transform, expand, and re-compose it into a new, complete visual interpretation that follows the visitor's direction. The result should clearly go beyond a cleaned-up or recoloured version of the drawing." : sourceFile ? "Use the first supplied photo as source material. Create a fully developed new visual interpretation from it. It may be expanded, transformed, and re-composed while remaining clearly connected to that photo." : "Make a visual starting point from the visitor's words without illustrating them literally.",
      ...
      aiPath === "translate" ? "Make the result visually developed and expressive. Follow the visitor's direction for subject, colour, atmosphere, and composition; you may introduce new visual elements when they serve that interpretation. Do not add text, letters, names, faces, people, religious symbols, memorial symbols, logos, or watermarks." : "Use restrained, natural colours and a simple composition. Do not add text, letters, names, faces, people, religious symbols, memorial symbols, flames, sparkles, dramatic light, logos, or watermarks.",
